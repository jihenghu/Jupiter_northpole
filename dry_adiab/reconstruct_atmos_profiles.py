...
def set_atmos_run_RT_concurrent(theta, tbs_shm_name, SH_NH3_shm_name, RH_NH3_shm_name, SH_H2O_shm_name, RH_H2O_shm_name, nstep, n_walkers):
    ...
    for i in range(len(aircolumn)):
        ...
        RH_NH3[int(istep), jindex, i] = ap_mole.hydro()[iNH3]*1.0E6   ## mole fraction [ppm]
        RH_H2O[int(istep), jindex, i] = ap_mole.hydro()[iH2O]*1.0E6
...
# H5OUT = h5py.File(f'reconstruct_atmos_tbs_last{nstep}.h5', 'w')
# H5OUT.create_dataset('tb', data=tbs)
# H5OUT.close()

profOUT = h5py.File(f'reconstruct_atmos_profiles_last{nstep}_LD101.h5', 'w')
